...
def getTweetTimestamp(tweet_id: int) -> datetime:
    unix_time_seconds = ((tweet_id >> 22) + 1288834974657) / 1000
...
def renameToNewDirectory(in_folder: str, out_folder: str):
    ...
    for file in file_list:
        if file[:19].isdigit():
            ...
            new_filename = getTweetTimestamp(
                twitter_id).strftime("%Y%m%d_%H%M%S")
            if os.path.exists(f"{out_folder}\\{new_filename + file_ext}"):
                ...
                while hasDuplicates:
                    dupe_count += 1
                    new_filename = (
                        getTweetTimestamp(twitter_id) +
                        timedelta(seconds=dupe_count)
                ...
                dupe_count = 0
            shutil.copy(
                f"{in_folder}\\{file}", f"{out_folder}\\{
                    new_filename + file_ext}"
            )
            success_count += 1
    print(
        f"---\nDone! {success_count} file(s) successfully copied and renamed. Check folder '{
            out_folder}' for renamed files."
    )
